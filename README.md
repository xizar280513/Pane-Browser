...
Pane Browser v6.0.0 is a cross-platform expansion of the Regular Edition with a local Password Manager and a Linux implementation. It also preserves the strict separation between Regular Edition and Mini Edition. The release does **not** include the previously prototyped DNS Settings feature; all DNS UI, DNS persistence, DoH arguments, and DNS handlers were removed from the v6 source and release packages.
...
The Regular Edition is the full Pane Browser experience. It combines the earlier navigation, tabs, persistence, privacy, recovery, profile, and productivity work with the v6 local Password Manager and Linux support.
...
The v6 Password Manager is available only in the Regular Edition. It supports both manual entry and automatic login detection. A saved entry contains four user-facing fields:
...
v6.0.0 provides exactly ten logical release assets:
...
```bash
bash windows/regular/build_v6_windows_regular.sh
```
...
```bash
bash windows/mini/build_v6_windows_mini.sh
```
...
The v6 source and package workflow includes static feature regression checks, exact-asset checks, clean source-payload checks, cross-compilation checks for Windows x64, Linux compilation, and headless Linux startup smoke tests in the development sandbox. The tests confirm that Mini source packages do not contain the Regular Password Manager or other Regular-only additions.
