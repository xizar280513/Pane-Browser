...
pane_v6::PasswordStore g_v6PasswordStore;
...
bool ConfirmV6PasswordSave(const std::wstring& origin) {
    std::wstring content = L"Pane Browser detected a login password for:\n" + origin + L"\n\nChoose Yes to Save or No to Do not save.";
...
void HandleV6PasswordMessage(BrowserTab* tab, ICoreWebView2WebMessageReceivedEventArgs* args) {
    ...
    if (ConfirmV6PasswordSave(canonicalOrigin)) {
        if (!g_v6PasswordStore.Save(V6Utf8FromWide(canonicalOrigin), pane_v6::DefaultPasswordName(V6Utf8FromWide(canonicalOrigin)), V6Utf8FromWide(username), V6Utf8FromWide(password))) MessageBoxW(g_main, L"The password could not be saved in the operating system credential store.", L"Password Manager", MB_OK | MB_ICONWARNING);
    }
}
...
void DeleteActiveProfile() {
    ...
    if (g_activeProfile.kind != L"normal" || g_activeProfile.temporary) {
        MessageBoxW(g_main, L"This profile type is temporary or protected and cannot be deleted.", L"Pane Browser", MB_OK | MB_ICONINFORMATION);
        return;
    }
...
std::wstring PasswordsHtml() {
    ...
    auto records = g_v6PasswordStore.List();
    ...
    for (size_t i = 0; i < records.size(); ++i) {
        ...
        if (name.empty()) name = V6WideFromUtf8(pane_v6::DefaultPasswordName(record.origin));
...
void HandleHistoryMessage(BrowserTab* tab, ICoreWebView2WebMessageReceivedEventArgs* args) {
    ...
    if (!message.empty() && message.front() == L'{') {
        ...
        if (ParseV6PasswordJson(message, type, origin, name, username, password) && (type == L"password-save" || type == L"password-update")) {
            ...
            if (valid && g_v6PasswordStore.Save(V6Utf8FromWide(canonicalOrigin), V6Utf8FromWide(name), V6Utf8FromWide(username), V6Utf8FromWide(password))) {
                if (type == L"password-update" && (OriginFromUri(oldOrigin) != canonicalOrigin || V6Utf8FromWide(oldUsername) != V6Utf8FromWide(username))) g_v6PasswordStore.Remove(V6Utf8FromWide(OriginFromUri(oldOrigin)), "", V6Utf8FromWide(oldUsername));
    ...
    if (message.rfind(L"v6-password-delete-selected:", 0) == 0) {
        ...
        auto records = g_v6PasswordStore.List();
        std::sort(indices.begin(), indices.end(), std::greater<int>());
        for (int index : indices) if (index >= 0 && index < static_cast<int>(records.size())) g_v6PasswordStore.Remove(records[static_cast<size_t>(index)].origin, records[static_cast<size_t>(index)].password_name, records[static_cast<size_t>(index)].username);
        ShowV6Passwords(); return;
    }
    if (message.rfind(L"v6-password-delete:", 0) == 0) {
        const wchar_t* start = message.c_str() + 19; wchar_t* end = nullptr; long index = wcstol(start, &end, 10); auto records = g_v6PasswordStore.List(); if (end && *end == L'\0' && index >= 0 && index < static_cast<long>(records.size())) g_v6PasswordStore.Remove(records[static_cast<size_t>(index)].origin, records[static_cast<size_t>(index)].password_name, records[static_cast<size_t>(index)].username); ShowV6Passwords(); return;
    }
...
bool CheckWebView2Runtime() { LPWSTR version=nullptr; HRESULT hr=GetAvailableCoreWebView2BrowserVersionString(nullptr,&version); if(SUCCEEDED(hr)&&version){CoTaskMemFree(version);return true;} MessageBoxW(g_main,(L"Microsoft WebView2 Runtime is required. Install the Evergreen WebView2 Runtime, then start Pane Browser again.\n\nHRESULT: " + HResultLabel(hr)).c_str(),L"Pane Browser",MB_OK|MB_ICONERROR); if(version)CoTaskMemFree(version); return false; }
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    ...
    case kUpdateResultMessage: {
        ...
        if (result->tag == L"v5.0.0" || result->tag == L"5.0.0") { MessageBoxW(g_main, L"You are using the latest Pane Browser release (v5.0.0).", L"Pane Browser", MB_OK | MB_ICONINFORMATION); return 0; }
...
int WINAPI wWinMain(HINSTANCE instance, HINSTANCE, PWSTR, int show) {
    ...
    if (FAILED(hr)) { MessageBoxW(g_main, (L"WebView2 initialization failed.\n\nHRESULT: " + HResultLabel(hr)).c_str(), L"Pane Browser", MB_OK | MB_ICONERROR); DestroyWindow(g_main); CoUninitialize(); return 1; }
