...
cd "$ROOT"
CORE="PaneBrowserMiniCore-v6.0.0.exe"
FINAL="PaneBrowser-v6.0.0-Mini-Edition-For-Windows-Portable.exe"
rm -f "$FINAL" "$CORE" mini_launcher-res.o mini_heap.o "$SDK/x64/WebView2Loader.dll.lz4"
python3 compress_resource.py "$SDK/x64/WebView2Loader.dll" "$SDK/x64/WebView2Loader.dll.lz4" > /tmp/pane-browser-v6-win-mini-compress.log
...
sed "s/PaneBrowserMiniCore.exe/$CORE/g" mini_launcher.rc > .v6-mini-launcher.rc
"$WINDRES" .v6-mini-launcher.rc -O coff -o mini_launcher-res.o
...
rm -f mini_launcher-res.o "$CORE" mini_heap.o "$SDK/x64/WebView2Loader.dll.lz4" .v6-mini-launcher.rc
file "$FINAL"
