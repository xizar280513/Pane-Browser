...
void CreateWebView() {
    ...
    auto callback = Callback<ICoreWebView2CreateCoreWebView2ControllerCompletedHandler>([](HRESULT hr, ICoreWebView2Controller* controller) -> HRESULT {
        if (FAILED(hr) || !controller) { MessageBoxW(g_main, L"WebView2 could not create the browser view.", L"Pane Browser Mini Edition", MB_OK | MB_ICONERROR); return hr; }
    ...
    HRESULT hr = g_environment->CreateCoreWebView2Controller(g_main, callback); callback->Release(); if (FAILED(hr)) MessageBoxW(g_main, L"WebView2 could not create the browser controller.", L"Pane Browser Mini Edition", MB_OK | MB_ICONERROR);
}

HRESULT InitializeWebView() {
    if (!CreateEphemeralUserDataFolder()) return E_FAIL;
    auto callback = Callback<ICoreWebView2CreateCoreWebView2EnvironmentCompletedHandler>([](HRESULT hr, ICoreWebView2Environment* environment) -> HRESULT { if (FAILED(hr) || !environment) { MessageBoxW(g_main, L"Microsoft Edge WebView2 Runtime is required.", L"Pane Browser Mini Edition", MB_OK | MB_ICONERROR); return hr; } g_environment = environment; CreateWebView(); return S_OK; });
