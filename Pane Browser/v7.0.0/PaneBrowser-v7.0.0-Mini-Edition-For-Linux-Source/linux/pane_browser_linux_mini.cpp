...
void BuildUi() {
    window = gtk_window_new(GTK_WINDOW_TOPLEVEL); gtk_window_set_title(GTK_WINDOW(window), "Pane Browser Mini Edition v7.0.0 — Linux"); gtk_window_set_default_size(GTK_WINDOW(window), 1100, 760); g_signal_connect(window, "destroy", G_CALLBACK(gtk_main_quit), nullptr); g_signal_connect(window, "key-press-event", G_CALLBACK(OnKey), nullptr);
    ...
    // v7 Mini: maximum privacy + lightness (ephemeral context already)
    ...
    webkit_settings_set_user_agent(s,
        ...
        "(KHTML, like Gecko) Version/17.0 Safari/605.1.15 PaneBrowserMini/7.0.0");
