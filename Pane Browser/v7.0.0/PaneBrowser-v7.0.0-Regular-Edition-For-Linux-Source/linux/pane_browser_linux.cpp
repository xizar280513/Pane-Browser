...
std::vector<Tab*> tabs;
pane_v7::PasswordStore password_store;
...
void ApplyPrivacySettings(WebKitWebView* view) {
    if (!view) return;
    WebKitSettings* s = webkit_web_view_get_settings(view);
    // v7 privacy + lightness hardening (stable subset of WebKitGTK 4.1 APIs)
    ...
    webkit_settings_set_enable_webaudio(s, FALSE);
    ...
    webkit_settings_set_user_agent(s,
        ...
        "(KHTML, like Gecko) Version/17.0 Safari/605.1.15 PaneBrowser/7.0.0");
}
...
const char* DetectorScript() {
    return R"JS((()=>{if(window.__paneV7Detector)return;window.__paneV7Detector=true;function offer(f){try{const p=f.querySelector('input[type=password]');if(!p||!p.value)return;const u=f.querySelector('input[type=email],input[name*=user i],input[name*=login i],input[type=text]');if(!u||!u.value)return;const o=location.origin;if(!/^https?:$/.test(location.protocol))return;window.webkit.messageHandlers.pane.postMessage(JSON.stringify({type:'password-offer',origin:o,username:u.value,password:p.value}));}catch(e){}}document.addEventListener('submit',e=>offer(e.target),true);})())JS";
}
...
void HandlePasswordOffer(const std::string& message, Tab* tab) {
    ...
    if (std::string(type) == "password-offer" && tab && !tab->internal && origin == Origin(tab->last_uri) && std::string(origin).rfind("https://", 0) == 0 && *username && *password) {
        ...
        if (response == GTK_RESPONSE_YES && !password_store.Save(origin, pane_v7::DefaultPasswordName(origin), username, password)) {
...
static gboolean OnDecidePolicy(WebKitWebView* view, WebKitPolicyDecision* decision, WebKitPolicyDecisionType type, gpointer data) {
    ...
    // Block dangerous schemes from automatic navigation
    std::string u(uri);
    if (u.rfind("javascript:", 0) == 0 || u.rfind("vbscript:", 0) == 0) {
    ...
    if (tab) {
        tab->last_uri = uri;
        tab->internal = false;
        tab->source = u.rfind("view-source:", 0) == 0;
        UpdateAddress(tab);
    }
    return FALSE;
}
...
#endif
    if (event->keyval == GDK_KEY_F11) { static bool fs = false; fs = !fs; if (fs) gtk_window_fullscreen(GTK_WINDOW(window)); else gtk_window_unfullscreen(GTK_WINDOW(window)); return TRUE; }
    return FALSE;
}

static void BuildUi() {
    window = gtk_window_new(GTK_WINDOW_TOPLEVEL);
    gtk_window_set_title(GTK_WINDOW(window), PB_MINI ? "Pane Browser Mini Edition — Linux" : "Pane Browser Regular Edition v7.0.0 — Linux");
...
int main(int argc, char** argv) {
    gtk_init(&argc, &argv);
    (void)ConfigDir(); // ensure config dir exists (v7)
    BuildUi();
#if PB_MINI
    NewTab("dashboard");
#else
