...
#include "../../shared/v7_password_store.h"
...
constexpr int kContextViewSource = 0x5211;
constexpr int kContextInspect = 0x5212;
...
constexpr UINT kOpenViewSourceMessage = WM_APP + 0x62;
constexpr UINT kOpenDevToolsMessage = WM_APP + 0x63;
...
pane_v7::PasswordStore g_v7PasswordStore;
...
const wchar_t* kDashboardHtmlDark = LR"HTML(<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Pane Browser v7.0.0</title><style>
...
std::string V6Utf8FromWide(const std::wstring& value) {
...
std::wstring V6WideFromUtf8(const std::string& value) {
    if (value.empty()) return {};
...
const wchar_t* kV6PasswordDetectorScript = LR"JS((()=>{
if(window.__paneV6PasswordDetector)return;window.__paneV6PasswordDetector=true;
function offer(form){try{const p=form.querySelector('input[type=password]');if(!p||!p.value)return;const u=form.querySelector('input[type=email],input[name*=user i],input[name*=login i],input[type=text]');if(!u||!u.value)return;const o=location.origin;if(!/^https?:$/.test(location.protocol)||!window.chrome||!chrome.webview)return;chrome.webview.postMessage(JSON.stringify({type:'password-offer',origin:o,username:u.value,password:p.value}))}catch(e){}}
document.addEventListener('submit',e=>offer(e.target),true);
window.addEventListener('beforeunload',()=>document.querySelectorAll('form').forEach(offer));
})())JS";

void InjectV6PasswordDetector(BrowserTab* tab) {
    if (!tab || !tab->webview || tab->privateMode || tab->internalPage) return;
    auto cb = Callback<ICoreWebView2ExecuteScriptCompletedHandler>([](HRESULT, LPCWSTR) { return S_OK; });
    tab->webview->ExecuteScript(kV6PasswordDetectorScript, cb.Get());
    cb.Get()->Release();
}

bool IsExternalHttpsSource(const std::wstring& source) {
    return source.rfind(L"https://", 0) == 0;
}

bool ExtractV6JsonString(const std::wstring& json, const wchar_t* field, std::wstring& out) {
    std::wstring key = L"\"" + std::wstring(field) + L"\"";
            ...
            switch (escaped) {
                ...
                case L'u': {
                    ...
                    for (size_t i = 0; i < 4; ++i) {
                        wchar_t h = json[pos++]; value <<= 4;
                        if (h >= L'0' && h <= L'9') value += static_cast<unsigned>(h - L'0');
                        else if (h >= L'a' && h <= L'f') value += static_cast<unsigned>(h - L'a' + 10);
                        else if (h >= L'A' && h <= L'F') value += static_cast<unsigned>(h - L'A' + 10);
                        else return false;
                    }
...
bool ParseV6PasswordJson(const std::wstring& message, std::wstring& type, std::wstring& origin, std::wstring& passwordName, std::wstring& username, std::wstring& password) {
    if (message.size() > 16384 || message.empty() || message.front() != L'{') return false;
    return ExtractV6JsonString(message, L"type", type) && ExtractV6JsonString(message, L"origin", origin) && ExtractV6JsonString(message, L"username", username) && ExtractV6JsonString(message, L"password", password) && (type == L"password-offer" || ExtractV6JsonString(message, L"passwordName", passwordName));
}

bool ConfirmV6PasswordSave(const std::wstring& origin) {
    std::wstring content = L"Pane Browser v7.0.0 detected a login password for:\n" + origin + L"\n\nChoose Yes to Save or No to Do not save.";
    return MessageBoxW(g_main, content.c_str(), L"Save password", MB_YESNO | MB_ICONQUESTION | MB_DEFBUTTON2) == IDYES;
}

void HandleV6PasswordMessage(BrowserTab* tab, ICoreWebView2WebMessageReceivedEventArgs* args) {
    if (!tab || !args || tab->privateMode || tab->internalPage) return;
    LPWSTR raw = nullptr;
    if (FAILED(args->TryGetWebMessageAsString(&raw)) || !raw) return;
    std::wstring message(raw);
    CoTaskMemFree(raw);
    std::wstring type, origin, passwordName, username, password;
    if (!ParseV6PasswordJson(message, type, origin, passwordName, username, password) || type != L"password-offer") return;
    std::wstring canonicalOrigin = OriginFromUri(origin), currentOrigin = OriginFromUri(tab->lastUri);
    if (!IsExternalHttpsSource(canonicalOrigin) || canonicalOrigin != currentOrigin || username.empty() || password.empty() || username.size() > 512 || password.size() > 4096) return;
    if (ConfirmV6PasswordSave(canonicalOrigin)) {
        if (!g_v7PasswordStore.Save(V6Utf8FromWide(canonicalOrigin), pane_v7::DefaultPasswordName(V6Utf8FromWide(canonicalOrigin)), V6Utf8FromWide(username), V6Utf8FromWide(password))) MessageBoxW(g_main, L"The password could not be saved in the operating system credential store.", L"Password Manager", MB_OK | MB_ICONWARNING);
    }
}
...
void SavePersistentData() {
    ...
    g_dataDirectory = g_profileDataDirectory;
    if (g_dataDirectory.empty()) { OutputDebugStringW(L"Pane Browser v7.0.0: no writable data directory.\n"); return; }
    bool ok = true;
    ...
    for (size_t i = 0; i < g_tabs.size(); ++i) { const auto& tab = g_tabs[i]; if (!tab->privateMode && !tab->authenticationWindow) { session.push_back({tab->lastUri, false}); if (static_cast<int>(i) == g_activeTab) normalActive = normalIndex; ++normalIndex; } }
    if (g_settings.restoreSession) {
        ok = v3data::SaveSession(session, normalActive, g_dataDirectory) && ok;
        std::vector<std::string> sessionMetadata;
        for (const auto& tab : g_tabs) if (tab && !tab->privateMode && !tab->authenticationWindow && (tab->lastUri.rfind(L"http://", 0) == 0 || tab->lastUri.rfind(L"https://", 0) == 0)) sessionMetadata.push_back(v4data::Encode(tab->lastUri) + "\t" + v4data::Encode(tab->workspaceId) + "\t" + v4data::Encode(tab->groupId));
        ok = v4data::WriteLines(v4data::Join(g_profileDataDirectory, L"session-meta.pbv"), sessionMetadata) && ok;
    } else {
        v3data::ClearSession(g_dataDirectory);
    ...
    SaveQuickNote();
    SaveSitePolicies();
    if (!ok) OutputDebugStringW(L"Pane Browser v7.0.0: one or more local data files could not be written.\n");
}
...
void ClearSessionMetadata() {
    if (!g_profileDataDirectory.empty()) DeleteFileW(v4data::Join(g_profileDataDirectory, L"session-meta.pbv").c_str());
}

void LoadPersistentData() {
    LoadProfileContext();
    g_settings = v3data::Settings{};
    g_bookmarks.clear();
    g_downloads.clear();
    LoadSitePolicies();
    LoadQuickNote();
...
void ResizeChildren();
void ApplyBrowserFullscreenState();
void ToggleBrowserFullscreen();
void RegisterFullscreenHandler(const std::shared_ptr<BrowserTab>& tab);
void CreateNewTab(const std::wstring& initialUri, bool privateMode = false, std::shared_ptr<BrowserTab> replaceTab = nullptr);
...
void SleepInactiveTabs() {
    ...
    for (size_t i = 0; i < g_tabs.size(); ++i) {
        ...
        ComPtr<ICoreWebView2_3> web3;
        if (SUCCEEDED(tab->webview->QueryInterface(IID_ICoreWebView2_3, reinterpret_cast<void**>(web3.GetAddressOf()))) && web3) {
            std::shared_ptr<BrowserTab> keepAlive = g_tabs[i];
            auto suspendCallback = Callback<ICoreWebView2TrySuspendCompletedHandler>([keepAlive](HRESULT errorCode, BOOL result) -> HRESULT {
                if (SUCCEEDED(errorCode) && result && FindTab(keepAlive.get()) >= 0) { keepAlive->sleeping = true; InvalidateTabs(); }
                return S_OK;
            });
...
void CloseTab(int index) {
    if (index < 0 || index >= static_cast<int>(g_tabs.size())) return;
    if (g_tabs.size() == 1 && (g_tabs[0]->privateMode || g_tabs[0]->authenticationWindow)) {
        if (g_tabs[0]->controller) g_tabs[0]->controller->Close();
        g_tabs[0]->webview.Reset();
        g_tabs[0]->controller.Reset();
        g_tabs.clear();
        g_activeTab = -1;
    ...
    if (g_tabs.size() == 1) {
        ...
        if (!tab->privateMode && !tab->authenticationWindow && (tab->lastUri.rfind(L"http://", 0) == 0 || tab->lastUri.rfind(L"https://", 0) == 0)) { g_closedTabs.push_back(tab->lastUri); if (g_closedTabs.size() > 10) g_closedTabs.erase(g_closedTabs.begin()); }
        ResumeTab(tab);
        ...
        tab->readingMode = false;
        tab->crashed = false;
        tab->videoFullscreen = false;
        tab->authenticationWindow = false;
        tab->privateMode = false;
        tab->guestMode = false;
        tab->workspaceId = L"default";
        tab->groupId.clear();
    ...
    if (!g_tabs[index]->privateMode && !g_tabs[index]->authenticationWindow && (g_tabs[index]->lastUri.rfind(L"http://", 0) == 0 || g_tabs[index]->lastUri.rfind(L"https://", 0) == 0)) { g_closedTabs.push_back(g_tabs[index]->lastUri); if (g_closedTabs.size() > 10) g_closedTabs.erase(g_closedTabs.begin()); }
    if (g_splitView) {
...
void FinalizeProfileDeletion() {
    ...
    MessageBoxW(g_main, message.c_str(), L"Pane Browser v7.0.0", MB_OK | ((appDataRemoved && g_profileDeleteWebDataSucceeded) ? MB_ICONINFORMATION : MB_ICONWARNING));
}

void DeleteActiveProfile() {
    if (g_profileDeletePending) return;
    if (_wcsicmp(g_activeProfile.id.c_str(), L"personal") == 0) {
        MessageBoxW(g_main, L"The Personal profile is protected and cannot be deleted.", L"Pane Browser v7.0.0", MB_OK | MB_ICONINFORMATION);
        return;
    }
    if (g_activeProfile.kind != L"normal" || g_activeProfile.temporary) {
        MessageBoxW(g_main, L"This profile type is temporary or protected and cannot be deleted.", L"Pane Browser v7.0.0", MB_OK | MB_ICONINFORMATION);
        return;
    }
    ...
    if (FAILED(hr)) {
        ...
        g_profileDeleteTarget.clear();
        if (g_environment) CreateNewTab(L"dashboard", false);
        MessageBoxW(g_main, L"WebView2 could not delete this profile because it is still in use or unavailable. Nothing was removed from Pane Browser v7.0.0 data.", L"Delete Profile", MB_OK | MB_ICONWARNING);
        return;
    }
...
void DeleteAllData() {
    ...
    if (!IsApprovedPaneDataRoot(root)) {
        MessageBoxW(g_main, L"Pane Browser v7.0.0 could not verify its local data folder. Nothing was deleted.", L"Delete All Data", MB_OK | MB_ICONWARNING);
        return;
    }
    std::wstring warning = L"Permanently delete ALL Pane Browser v7.0.0 data?\\n\\nThis removes the entire folder:\\n" + root + L"\\n\\nIt includes every profile, History, Bookmarks, Downloads records, Settings, sessions, permissions, cookies, cache, WebView2 website data, and local sign-in state. This cannot be recovered, restored, or backed up by Pane Browser v7.0.0. Files saved outside this folder, such as downloads in a custom external directory, are not deleted. Pane Browser v7.0.0 will close immediately.";
    if (MessageBoxW(g_main, warning.c_str(), L"Delete All Data", MB_YESNO | MB_ICONWARNING | MB_DEFBUTTON2) != IDYES) return;
    if (MessageBoxW(g_main, L"Final confirmation: permanently erase the entire Pane Browser v7.0.0 data folder now? There is no undo and no recovery inside Pane Browser v7.0.0.", L"Delete All Data", MB_YESNO | MB_ICONWARNING | MB_DEFBUTTON2) != IDYES) return;
    if (!LaunchDeferredRootDeletion()) {
        MessageBoxW(g_main, L"Pane Browser v7.0.0 could not start the safe cleanup process. Nothing was deleted.", L"Delete All Data", MB_OK | MB_ICONERROR);
        return;
    }
...
const wchar_t* kV6OpenEyeSvg = LR"SVG(<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7-10-7-10-7z"/><circle cx="12" cy="12" r="3"/></svg>)SVG";
const wchar_t* kV6ClosedEyeSvg = LR"SVG(<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24"/><line x1="1" y1="1" x2="23" y2="23"/></svg>)SVG";

std::wstring PasswordsHtml() {
    const wchar_t* bg = g_darkMode ? L"#202124" : L"#f8f9fa";
    const wchar_t* card = g_darkMode ? L"#292a2d" : L"#ffffff";
    const wchar_t* textColor = g_darkMode ? L"#f1f3f4" : L"#202124";
    const wchar_t* muted = g_darkMode ? L"#bdc1c6" : L"#5f6368";
    std::wstring html = L"<!doctype html><html><head><meta charset='utf-8'><style>html,body{min-height:100%;}body{margin:0;font-family:Segoe UI,system-ui,sans-serif;background:" + std::wstring(bg) + L";color:" + textColor + L"}.container{max-width:1250px;margin:auto;padding:36px 28px}.card{background:" + card + L";border-radius:16px;padding:28px 26px 36px;box-shadow:0 0 36px rgba(0,0,0,.25)}h1{margin:0 0 22px;font-size:34px;font-weight:600}.search{width:100%;box-sizing:border-box;background:" + std::wstring(g_darkMode ? L"#303134" : L"#fff") + L";color:" + textColor + L";border:1px solid " + std::wstring(g_darkMode ? L"#5f6368" : L"#c7cbd1") + L";border-radius:26px;padding:13px 20px;font-size:17px;outline:none}.form{display:grid;grid-template-columns:1.1fr 1.05fr 1fr 1fr auto;gap:10px;margin:20px 0 10px}.pwfield{display:flex;align-items:center;border:1px solid #777;border-radius:8px;background:inherit;overflow:hidden}.pwfield input{border:0!important;flex:1;min-width:0;outline:none!important;box-shadow:none!important}.eye{width:40px;height:40px;border:0;background:transparent;color:inherit;cursor:pointer;font-size:0;display:inline-flex;align-items:center;justify-content:center}.eye svg{width:18px;height:18px;display:block;pointer-events:none}input[type=password]::-ms-reveal,input[type=password]::-ms-clear{display:none}input[type=password]::-webkit-credentials-auto-fill-button{display:none!important;visibility:hidden;pointer-events:none}.form input{box-sizing:border-box;width:100%;padding:11px 12px;border-radius:8px;border:1px solid #777;background:" + std::wstring(bg) + L";color:" + textColor + L";font:15px Segoe UI,system-ui,sans-serif;outline:none}.button{border:0;border-radius:8px;padding:10px 13px;background:" + std::wstring(g_darkMode ? L"#3c4043" : L"#e8eaed") + L";color:" + textColor + L";cursor:pointer;font:14px Segoe UI,system-ui,sans-serif}.save{background:#8ab4f8;color:#202124}.hint{color:" + muted + L";font-size:13px;margin:0 0 12px}.selection{display:flex;align-items:center;gap:12px;padding:14px 2px 8px;color:" + textColor + L"}.selection .bulk-action{display:none}.selection.has-selection .bulk-action{display:inline-block}.selection button{background:transparent;color:" + (g_darkMode ? L"#8ab4f8" : L"#1967d2") + L";padding:8px 10px;border:0;border-radius:6px;cursor:pointer}.selection button:hover{background:" + (g_darkMode ? L"#3c4043" : L"#f1f3f4") + L"}.row{display:grid;grid-template-columns:28px 34px minmax(130px,1fr) minmax(170px,1.15fr) minmax(150px,1fr) minmax(150px,1fr) 42px 72px 72px;align-items:center;gap:10px;min-height:74px;border-bottom:1px solid " + std::wstring(g_darkMode ? L"#3c4043" : L"#e8eaed") + L"}.pick{width:18px;height:18px;accent-color:#8ab4f8}.siteicon{width:28px;height:28px;border-radius:50%;display:flex;align-items:center;justify-content:center;background:#5f6368;color:#fff;font-weight:700}.origin,.name,.username{white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.muted{color:" + muted + L";font-size:13px}.secretwrap{min-width:0}.secret{width:100%;box-sizing:border-box;background:transparent;color:" + textColor + L";border:0;font:16px Segoe UI,system-ui,sans-serif;min-width:0;outline:none!important;box-shadow:none!important}.secret:focus{outline:none!important;box-shadow:none!important}.empty{color:" + muted + L";padding:22px 0;font-size:16px}.cancel{display:none}@media(max-width:1000px){.form{grid-template-columns:1fr 1fr}.row{grid-template-columns:28px 30px 1fr 1fr 1fr;gap:8px;padding:12px 0}.row .secretwrap{grid-column:3/5}.row .edit,.row .delete{grid-column:auto}}</style></head><body><main class='container'><section class='card'><h1>Saved Passwords</h1><input id='filter' class='search' aria-label='Search saved passwords' placeholder='Search saved passwords' oninput=\"filterRows()\"><form class='form' onsubmit=\"saveManual();return false\"><input id='origin' required placeholder='https://example.com'><input id='name' required placeholder='Password name'><input id='username' required placeholder='Username'><div class='pwfield'><input id='password' required type='password' placeholder='Password'><button class='eye' type='button' aria-label='Show password' onclick=\"togglePassword(this,document.getElementById('password'))\">";
    html += kV6ClosedEyeSvg;
    html += L"</button></div><button id='saveButton' class='button save' type='submit'>Save password</button><button id='cancelButton' class='button cancel' type='button' onclick='cancelEdit()'>Cancel</button></form><p class='hint'>Passwords are stored in the operating system credential store. You can add them manually or save them after a detected login.</p><div id='selection' class='selection'><span id='count'>0 selected</span><button type='button' onclick='selectAll()'>Select all</button><button class='bulk-action' type='button' onclick='deselectAll()'>Deselect all</button><button class='bulk-action' type='button' onclick='deleteSelected()'>Delete selected</button></div><div id='rows'>";
    auto records = g_v7PasswordStore.List();
    if (records.empty()) html += L"<div class='empty'>No saved passwords yet.</div>";
    for (size_t i = 0; i < records.size(); ++i) {
        const auto& record = records[i];
        std::wstring origin = V6WideFromUtf8(record.origin), name = V6WideFromUtf8(record.password_name), username = V6WideFromUtf8(record.username), password = V6WideFromUtf8(record.password);
        if (name.empty()) name = V6WideFromUtf8(pane_v7::DefaultPasswordName(record.origin));
        if (password.empty()) password = L"(password unavailable)";
        html += L"<div class='row' data-password-row data-index='" + std::to_wstring(i) + L"' data-origin='" + HtmlEscape(origin) + L"' data-name='" + HtmlEscape(name) + L"' data-username='" + HtmlEscape(username) + L"' data-password='" + HtmlEscape(password) + L"' data-search='" + HtmlEscape(name + L" " + origin + L" " + username) + L"'><input type='checkbox' class='pick' onchange='syncSelection()'><div class='siteicon'>P</div><div class='name' title='" + HtmlEscape(name) + L"'><strong>" + HtmlEscape(name) + L"</strong><div class='muted'>Password name</div></div><div class='origin' title='" + HtmlEscape(origin) + L"'><strong>" + HtmlEscape(origin) + L"</strong><div class='muted'>Website</div></div><div class='username' title='" + HtmlEscape(username) + L"'><strong>" + HtmlEscape(username) + L"</strong><div class='muted'>Username</div></div><div class='secretwrap'><input class='secret' type='password' value='" + HtmlEscape(password) + L"' readonly><div class='muted'>Password</div></div><button class='button eye saved-eye' aria-label='Show password' type='button' onclick=\"toggleSavedPassword(this)\">";
        html += kV6ClosedEyeSvg;
        html += L"</button><button class='button edit' type='button' onclick='editRow(this)'>Edit</button><button class='button delete' type='button' onclick=\"send('v6-password-delete:" + std::to_wstring(i) + L"')\">Delete</button></div>";
    }
    html += L"</div></section></main><script>const send=m=>window.chrome&&chrome.webview&&chrome.webview.postMessage(m);const OPEN_EYE_SVG='";
    html += kV6OpenEyeSvg;
    html += L"';const CLOSED_EYE_SVG='";
    html += kV6ClosedEyeSvg;
    html += L"';let editingIndex=-1;function setEye(b,visible){b.innerHTML=visible?OPEN_EYE_SVG:CLOSED_EYE_SVG;b.setAttribute('aria-label',visible?'Hide password':'Show password')}function togglePassword(b,p){const visible=p.type==='password';p.type=visible?'text':'password';setEye(b,visible)}function toggleSavedPassword(b){togglePassword(b,b.parentElement.querySelector('.secret'))}function rows(){return [...document.querySelectorAll('[data-password-row]')]}function visibleRows(){return rows().filter(r=>r.style.display!=='none')}function syncSelection(){const picks=document.querySelectorAll('.pick:checked');document.getElementById('count').textContent=picks.length+' selected';document.getElementById('selection').classList.toggle('has-selection',picks.length>0)}function selectAll(){visibleRows().forEach(r=>{r.querySelector('.pick').checked=true});syncSelection()}function deselectAll(){visibleRows().forEach(r=>{r.querySelector('.pick').checked=false});syncSelection()}function deleteSelected(){const ids=rows().filter(r=>r.querySelector('.pick').checked).map(r=>r.dataset.index);if(ids.length)send('v6-password-delete-selected:'+ids.join(','))}function filterRows(){const q=document.getElementById('filter').value.toLowerCase();rows().forEach(r=>{const visible=r.dataset.search.toLowerCase().includes(q);r.style.display=visible?'grid':'none';if(!visible)r.querySelector('.pick').checked=false});syncSelection()}function editRow(b){const r=b.closest('[data-password-row]');editingIndex=Number(r.dataset.index);document.getElementById('origin').value=r.dataset.origin;document.getElementById('name').value=r.dataset.name;document.getElementById('username').value=r.dataset.username;const p=document.getElementById('password');p.value=r.dataset.password;p.type='password';document.getElementById('saveButton').textContent='Update password';document.getElementById('cancelButton').style.display='inline-block';document.getElementById('origin').focus();window.scrollTo(0,0)}function cancelEdit(){editingIndex=-1;document.querySelector('.form').reset();document.getElementById('password').type='password';document.getElementById('saveButton').textContent='Save password';document.getElementById('cancelButton').style.display='none'}function saveManual(){const o=document.getElementById('origin').value.trim(),n=document.getElementById('name').value.trim(),u=document.getElementById('username').value,p=document.getElementById('password').value;if(!o||!n||!u||!p)return;const payload={type:editingIndex>=0?'password-update':'password-save',origin:o,passwordName:n,username:u,password:p};if(editingIndex>=0){const r=rows().find(x=>Number(x.dataset.index)===editingIndex);if(r){payload.oldOrigin=r.dataset.origin;payload.oldUsername=r.dataset.username}}send(JSON.stringify(payload))}</script></body></html>";
    return html;
}

void ShowV6Passwords() { BrowserTab* tab = ActiveTab(); if (!tab || !tab->webview) return; ClearInternalFlags(tab); tab->internalPage = true; tab->passwordPage = true; SetWindowTextW(g_address, L"Saved Passwords"); tab->webview->NavigateToString(PasswordsHtml().c_str()); }

void ShowNotes() { BrowserTab* tab = ActiveTab(); if (!tab || !tab->webview) return; ClearInternalFlags(tab); tab->internalPage = true; tab->notesPage = true; SetWindowTextW(g_address, L"Quick Notes"); tab->webview->NavigateToString(NotesHtml().c_str()); }
void ShowSiteData() { BrowserTab* tab = ActiveTab(); if (!tab || !tab->webview) return; ClearInternalFlags(tab); tab->internalPage = true; tab->siteDataPage = true; SetWindowTextW(g_address, L"Site Data Center"); tab->webview->NavigateToString(SiteDataHtml().c_str()); }
void ShowWorkspace() { BrowserTab* tab = ActiveTab(); if (!tab || !tab->webview) return; ClearInternalFlags(tab); tab->internalPage = true; tab->workspacePage = true; SetWindowTextW(g_address, L"Workspace"); tab->webview->NavigateToString(WorkspaceHtml().c_str()); }
...
void BackupProfile() {
    BROWSEINFOW bi{}; bi.hwndOwner = g_main; bi.lpszTitle = L"Choose a folder for the local Pane Browser v7.0.0 backup"; bi.ulFlags = BIF_RETURNONLYFSDIRS | BIF_NEWDIALOGSTYLE;
    ...
    std::wstring destination = v4data::Join(selected, L"PaneBrowser-v5-backup");
    ...
    MessageBoxW(g_main, copied ? L"App-owned profile data was backed up. Cookies, tokens, and WebView2 UDF data were not exported." : L"Some profile files could not be backed up.", L"Pane Browser v7.0.0", MB_OK | (copied ? MB_ICONINFORMATION : MB_ICONWARNING));
}

void RestoreProfileBackup() {
    BROWSEINFOW bi{}; bi.hwndOwner = g_main; bi.lpszTitle = L"Choose a Pane Browser v7.0.0 app-data backup folder"; bi.ulFlags = BIF_RETURNONLYFSDIRS | BIF_NEWDIALOGSTYLE;
    PIDLIST_ABSOLUTE pidl = SHBrowseForFolderW(&bi); if (!pidl) return;
    wchar_t selected[MAX_PATH] = {}; bool ok = SHGetPathFromIDListW(pidl, selected) != FALSE; CoTaskMemFree(pidl); if (!ok) return;
    if (MessageBoxW(g_main, L"Restore app-owned History, Bookmarks, Settings, Session, Downloads, and Notes into the active profile? WebView2 cookies and login data will not be imported.", L"Pane Browser v7.0.0", MB_YESNO | MB_ICONWARNING) != IDYES) return;
    const wchar_t* files[] = {L"history.pbh", L"bookmarks.pbb", L"settings.pbs", L"session.pbs", L"session-meta.pbv", L"downloads.pbd", L"quick-note.pbn"};
...
std::wstring SettingsHtml() {
    ...
    bool deletableActiveProfile = g_activeProfile.kind == L"normal" && !g_activeProfile.temporary && _wcsicmp(g_activeProfile.id.c_str(), L"personal") != 0;
    const wchar_t* protectedProfileLabel = _wcsicmp(g_activeProfile.id.c_str(), L"personal") == 0 ? L"Personal profile is protected" : L"This profile type is protected";
    html += L"<div class='setting'><span>Active profile</span><select onchange=\"send('profile-switch:'+this.value)\">" + ProfileOptionsHtml() + L"</select></div><div class='setting'><span>Create profile</span><input id='newProfile' style='width:40%' placeholder='Work or School'><button onclick=\"send('profile-create:'+document.getElementById('newProfile').value)\">Create</button><button onclick=\"send('guest-session')\">Open Guest Session</button></div><div class='setting'><span>Delete profile</span>" + (deletableActiveProfile ? L"<button onclick=\"send('profile-delete:" + HtmlEscape(g_activeProfile.id) + L"')\">Delete Profile</button>" : L"<button disabled title='" + std::wstring(protectedProfileLabel) + L"'>" + protectedProfileLabel + L"</button>") + L"</div><div class='setting'><span>Search engine</span><select onchange=\"send('settings-search-engine:'+this.value)\">" + SearchEngineOptionsHtml() + L"</select></div><div class='setting'><span>Custom search URL</span><input style='width:52%' value='" + HtmlEscape(g_settings.customSearchUrl) + L"' placeholder='https://example.com/search?q={query}' onchange=\"send('settings-custom-search:'+this.value)\"></div>";
    html += L"<div class='setting'><span>Password Manager</span><button onclick=\"send('v6-passwords')\">View saved passwords</button></div>";
...
bool IsTrustedInternalMessage(BrowserTab* tab, ICoreWebView2WebMessageReceivedEventArgs* args) {
    if (!tab || !args || !(tab->internalPage || tab->historyPage || tab->bookmarksPage || tab->settingsPage || tab->downloadsPage || tab->commandPalettePage || tab->notesPage || tab->siteDataPage || tab->workspacePage || tab->passwordPage)) return false;
...
void HandleHistoryMessage(BrowserTab* tab, ICoreWebView2WebMessageReceivedEventArgs* args) {
    ...
    if (message == L"v6-passwords") { ShowV6Passwords(); return; }
    ...
    if (message == L"palette-passwords") { ShowV6Passwords(); return; }
    ...
    if (!message.empty() && message.front() == L'{') {
        std::wstring type, origin, name, username, password;
        if (ParseV6PasswordJson(message, type, origin, name, username, password) && (type == L"password-save" || type == L"password-update")) {
            std::wstring canonicalOrigin = OriginFromUri(origin), oldOrigin, oldUsername;
            bool valid = origin.size() <= 2048 && name.size() <= 512 && username.size() <= 512 && password.size() <= 4096 && IsExternalHttpsSource(canonicalOrigin) && !canonicalOrigin.empty() && !name.empty() && !username.empty() && !password.empty() && origin.find_first_of(L"\t\r\n") == std::wstring::npos && name.find_first_of(L"\t\r\n") == std::wstring::npos && username.find_first_of(L"\t\r\n") == std::wstring::npos && password.find_first_of(L"\r\n") == std::wstring::npos;
            if (type == L"password-update") valid = valid && ExtractV6JsonString(message, L"oldOrigin", oldOrigin) && ExtractV6JsonString(message, L"oldUsername", oldUsername) && !oldOrigin.empty() && !oldUsername.empty();
            if (valid && g_v7PasswordStore.Save(V6Utf8FromWide(canonicalOrigin), V6Utf8FromWide(name), V6Utf8FromWide(username), V6Utf8FromWide(password))) {
                if (type == L"password-update" && (OriginFromUri(oldOrigin) != canonicalOrigin || V6Utf8FromWide(oldUsername) != V6Utf8FromWide(username))) g_v7PasswordStore.Remove(V6Utf8FromWide(OriginFromUri(oldOrigin)), "", V6Utf8FromWide(oldUsername));
                ShowV6Passwords(); return;
            }
            MessageBoxW(g_main, L"The password could not be saved. Use an HTTPS origin, a password name, a username, and a non-empty password.", L"Password Manager", MB_OK | MB_ICONWARNING); return;
        }
    }
    if (message.rfind(L"v6-password-delete-selected:", 0) == 0) {
        std::vector<int> indices; size_t start = 28;
        while (start < message.size()) { size_t end = message.find(L',', start); try { indices.push_back(std::stoi(message.substr(start, end == std::wstring::npos ? std::wstring::npos : end - start))); } catch (...) {} if (end == std::wstring::npos) break; start = end + 1; }
        auto records = g_v7PasswordStore.List();
        std::sort(indices.begin(), indices.end(), std::greater<int>());
        for (int index : indices) if (index >= 0 && index < static_cast<int>(records.size())) g_v7PasswordStore.Remove(records[static_cast<size_t>(index)].origin, records[static_cast<size_t>(index)].password_name, records[static_cast<size_t>(index)].username);
        ShowV6Passwords(); return;
    }
    if (message.rfind(L"v6-password-delete:", 0) == 0) {
        const wchar_t* start = message.c_str() + 19; wchar_t* end = nullptr; long index = wcstol(start, &end, 10); auto records = g_v7PasswordStore.List(); if (end && *end == L'\0' && index >= 0 && index < static_cast<long>(records.size())) g_v7PasswordStore.Remove(records[static_cast<size_t>(index)].origin, records[static_cast<size_t>(index)].password_name, records[static_cast<size_t>(index)].username); ShowV6Passwords(); return;
    }
    ...
    if (message.rfind(L"settings-history:", 0) == 0) {
        g_settings.persistHistory = message.substr(17) != L"0";
        if (!g_settings.persistHistory) { g_history.clear(); v3data::ClearHistory(g_dataDirectory); }
        SavePersistentData();
        RefreshSettingsPage(); return;
    }
    if (message.rfind(L"settings-restore:",0)==0) {
        g_settings.restoreSession = message.substr(17) != L"0";
        if (!g_settings.restoreSession) { g_restoreSession.clear(); g_restoreTabMetadata.clear(); g_restoreActive = 0; v3data::ClearSession(g_dataDirectory); ClearSessionMetadata(); SavePersistentData(); }
        else SavePersistentData();
        RefreshSettingsPage(); return;
    }
...
void ShowPrivate() {
    if (g_settings.tabsEnabled) {
        CreateNewTab(L"dashboard", true);
    } else if (!g_tabs.empty()) {
        std::shared_ptr<BrowserTab> replaceTab = g_activeTab >= 0 && g_activeTab < static_cast<int>(g_tabs.size()) ? g_tabs[static_cast<size_t>(g_activeTab)] : g_tabs.front();
        CreateNewTab(L"dashboard", true, replaceTab);
    }
}
...
void ApplyBrowserFullscreenState() {
    ...
    bool shouldBeFullscreen = g_f11Fullscreen || (active && active->videoFullscreen);
    ...
    if (shouldBeFullscreen) {
        ...
        if (monitor && GetMonitorInfoW(monitor, &info)) {
            LONG_PTR style = g_fullscreenStyle & ~static_cast<LONG_PTR>(WS_CAPTION | WS_THICKFRAME | WS_MINIMIZEBOX | WS_MAXIMIZEBOX | WS_SYSMENU | WS_SIZEBOX);
    ...
    ResizeChildren();
    BrowserTab* restored = ActiveTab();
    if (!g_browserFullscreen && restored && g_address && !restored->internalPage) {
        SetWindowTextW(g_address, restored->lastUri == L"about:blank" ? L"" : restored->lastUri.c_str());
    }
...
void RegisterFullscreenHandler(const std::shared_ptr<BrowserTab>& tab) {
    if (!tab || !tab->webview) return;
    auto callback = Callback<ICoreWebView2ContainsFullScreenElementChangedEventHandler>([tab](ICoreWebView2* sender, IUnknown*) -> HRESULT {
        BOOL contains = FALSE;
        if (sender && SUCCEEDED(sender->get_ContainsFullScreenElement(&contains))) {
            tab->videoFullscreen = contains != FALSE;
            if (tab.get() == ActiveTab()) ApplyBrowserFullscreenState();
        }
        return S_OK;
    });
    EventRegistrationToken token{};
    tab->webview->add_ContainsFullScreenElementChanged(callback.Get(), &token);
...
void ResizeChildren() {
    ...
    if (g_browserFullscreen) {
        ShowWindow(g_tabStrip, SW_HIDE); ShowWindow(g_back, SW_HIDE); ShowWindow(g_forward, SW_HIDE); ShowWindow(g_reload, SW_HIDE); ShowWindow(g_home, SW_HIDE); ShowWindow(g_historyButton, SW_HIDE); ShowWindow(g_bookmarksButton, SW_HIDE); ShowWindow(g_settingsButton, SW_HIDE); ShowWindow(g_privateButton, SW_HIDE); ShowWindow(g_downloadsButton, SW_HIDE); ShowWindow(g_findButton, SW_HIDE); ShowWindow(g_address, SW_HIDE); ShowWindow(g_overflowButton, SW_HIDE);
        for (auto& tab : g_tabs) if (tab && tab->controller) { RECT full{0, 0, rc.right, rc.bottom}; tab->controller->put_Bounds(full); tab->controller->put_IsVisible(tab.get() == ActiveTab() ? TRUE : FALSE); }
        return;
    }
    ShowWindow(g_back, SW_SHOW); ShowWindow(g_forward, SW_SHOW); ShowWindow(g_reload, SW_SHOW); ShowWindow(g_home, SW_SHOW); ShowWindow(g_address, SW_SHOW); ShowWindow(g_overflowButton, SW_SHOW);
...
void ShowDownloads() { BrowserTab* tab = ActiveTab(); if (!tab || !tab->webview) return; ClearInternalFlags(tab); tab->internalPage = true; tab->downloadsPage = true; SetWindowTextW(g_address, L""); tab->webview->NavigateToString(DownloadsHtml().c_str()); }
...
void ExportBookmarks() {
    ...
    std::ofstream out(path, std::ios::binary|std::ios::trunc); if (!out) { MessageBoxW(g_main,L"Could not export bookmarks.",L"Pane Browser v7.0.0",MB_OK|MB_ICONERROR); return; }
    out << "<!DOCTYPE NETSCAPE-Bookmark-file-1>\\n<TITLE>Pane Browser v7.0.0 Bookmarks</TITLE>\\n<H1>Pane Browser v7.0.0 Bookmarks</H1>\\n<DL><p>\\n";
...
void ImportBookmarks() {
    ...
    SavePersistentData(); if(added==0)MessageBoxW(g_main,L"No new HTTP(S) bookmarks were found.",L"Pane Browser v7.0.0",MB_OK|MB_ICONINFORMATION); 
}
...
void ClearBrowsingData() {
    if (MessageBoxW(g_main, L"Clear local History, download records, cookies, cache, and site data? This may sign you out of websites.", L"Pane Browser v7.0.0", MB_YESNO | MB_ICONWARNING) != IDYES) return;
    g_history.clear(); g_downloads.clear(); g_closedTabs.clear();
    g_restoreSession.clear(); g_restoreTabMetadata.clear(); g_restoreActive = 0;
    g_sitePolicies.clear(); g_permissionPolicies.clear();
    v3data::ClearHistory(g_dataDirectory); v3data::SaveDownloads({}, g_dataDirectory); v3data::ClearSession(g_dataDirectory); ClearSessionMetadata(); SaveSitePolicies();
    ...
    if (profiles.empty()) { MessageBoxW(g_main, L"Local records were cleared, but this WebView2 Runtime does not expose profile data clearing.", L"Pane Browser v7.0.0", MB_OK | MB_ICONWARNING); RefreshHistoryPage(); return; }
    ...
    for (const auto& profile : profiles) {
        auto callback = Callback<ICoreWebView2ClearBrowsingDataCompletedHandler>([pending, failed](HRESULT hr) -> HRESULT {
            ...
            --*pending;
            if (*pending == 0) {
                MessageBoxW(g_main, *failed ? L"Some WebView2 browsing data could not be cleared." : L"Browsing history, download history, cookies, cache, and site data were cleared.", L"Pane Browser v7.0.0", MB_OK | (*failed ? MB_ICONWARNING : MB_ICONINFORMATION));
                RefreshHistoryPage();
            }
            return S_OK;
        });
    ...
    if (*pending == 0) { MessageBoxW(g_main, L"WebView2 browsing data could not be cleared.", L"Pane Browser v7.0.0", MB_OK | MB_ICONWARNING); RefreshHistoryPage(); }
}
...
DWORD WINAPI UpdateCheckWorker(LPVOID) {
    ...
    HINTERNET internet = InternetOpenW(L"PaneBrowser/5.0", INTERNET_OPEN_TYPE_PRECONFIG, nullptr, nullptr, INTERNET_FLAG_NO_CACHE_WRITE);
    if (internet) {
        DWORD timeout = 5000;
        ...
        const wchar_t* headers = L"User-Agent: PaneBrowser/5.0\r\nAccept: application/vnd.github+json\r\n";
...
void CheckForUpdates() {
    ...
    if (!thread) {
        ...
        MessageBoxW(g_main, L"Update check could not start.", L"Pane Browser v7.0.0", MB_OK | MB_ICONWARNING);
        return;
    }
    CloseHandle(thread);
}
...
void OpenViewSource() {
    BrowserTab* tab = ActiveTab();
    if (!tab || !tab->webview) return;
    std::wstring uri = tab->lastUri;
    if (uri.rfind(L"view-source:", 0) == 0) uri = uri.substr(12);
    if (uri.rfind(L"http://", 0) != 0 && uri.rfind(L"https://", 0) != 0) return;
    std::wstring sourceUri = L"view-source:" + uri;
    if (g_settings.tabsEnabled) CreateNewTab(sourceUri, tab->privateMode);
    else { ClearInternalFlags(tab); tab->webview->Navigate(sourceUri.c_str()); }
}

void OpenDevTools() {
    BrowserTab* tab = ActiveTab();
    if (tab && tab->webview) tab->webview->OpenDevToolsWindow();
}

bool HandleShortcut(UINT vk, bool ctrl, bool shift, bool alt) { if(alt)return false; if(!ctrl && vk==VK_F11){ToggleBrowserFullscreen();return true;} if(!ctrl && vk==VK_F12){PostMessageW(g_main,kOpenDevToolsMessage,0,0);return true;} if(!ctrl && vk==VK_F5){if(ActiveWebView())ActiveWebView()->Reload();return true;} if(!ctrl)return false; if(ctrl&&shift&&vk==VK_SPACE){ShowCommandPalette();return true;} if(ctrl&&shift&&vk=='T'){ReopenClosedTab();return true;} if(ctrl&&vk=='L'){SetFocus(g_address);SendMessageW(g_address,EM_SETSEL,0,-1);} else if(ctrl&&!shift&&vk=='U'){PostMessageW(g_main,kOpenViewSourceMessage,0,0);} else if(ctrl&&vk=='T'&&g_settings.tabsEnabled)CreateNewTab(L"dashboard",false); else if(ctrl&&vk=='W')CloseTab(g_activeTab); else if (ctrl && (vk == 'R' || vk == VK_F5)) { if(ActiveWebView()) ActiveWebView()->Reload(); } else if(ctrl&&vk==VK_TAB){int n=static_cast<int>(g_tabs.size());if(n)SetActiveTab((g_activeTab+(shift?-1:1)+n)%n);} else if(ctrl&&vk=='F')FindInPage(); else if(ctrl&&vk=='D')AddCurrentBookmark(); else if(ctrl&&vk=='J')ShowDownloads(); else if(ctrl&&vk=='H')ShowHistory(); else if(ctrl&&shift&&vk=='P')ShowPrivate(); else if(ctrl&&(vk==VK_OEM_PLUS||vk==VK_ADD))AdjustZoom(0.1); else if(ctrl&&(vk==VK_OEM_MINUS||vk==VK_SUBTRACT))AdjustZoom(-0.1); else if(ctrl&&vk=='0')ResetZoom(); else return false; return true; }
...
void HandleManagedNewWindowRequest(BrowserTab* opener, ICoreWebView2* sender, ICoreWebView2NewWindowRequestedEventArgs* args) {
    if (!opener || !opener->webview || !sender || !args) return;
    LPWSTR rawUri = nullptr;
    ...
    bool authenticationPopup = blankPopup || IsAuthenticationUri(requestedUri);
    bool sourceDocument = opener->lastUri.size() > 12 && _wcsnicmp(opener->lastUri.c_str(), L"view-source:", 12) == 0;
    bool httpLink = requestedUri.rfind(L"http://", 0) == 0 || requestedUri.rfind(L"https://", 0) == 0;
    if (!authenticationPopup && userInitiated && sourceDocument && httpLink && g_settings.tabsEnabled) {
        args->put_Handled(TRUE);
        CreateNewTab(requestedUri, opener->privateMode);
        return;
    }
    if (!authenticationPopup && userInitiated) {
        args->put_Handled(TRUE);
        ClearInternalFlags(opener);
        opener->webview->Navigate(requestedUri.c_str());
        return;
    }
    if (!authenticationPopup && !userInitiated) {
        args->put_Handled(TRUE);
    ...
    popup->authenticationWindow = authenticationPopup;
    ...
    auto callback = Callback<ICoreWebView2CreateCoreWebView2ControllerCompletedHandler>(
        [popup, argsHold, deferral, requestedUri, discardPopup](HRESULT result, ICoreWebView2Controller* controller) -> HRESULT {
            ...
            RegisterFullscreenHandler(popup);
            ...
            auto navigationCallback = Callback<ICoreWebView2NavigationCompletedEventHandler>(
                [popup](ICoreWebView2*, ICoreWebView2NavigationCompletedEventArgs*) -> HRESULT {
                    ...
                    InjectV6PasswordDetector(popup.get());
                    return S_OK;
                });
...
void CreateNewTab(const std::wstring& initialUri, bool privateMode, std::shared_ptr<BrowserTab> replaceTab) {
    ...
    auto controllerCallback = Callback<ICoreWebView2CreateCoreWebView2ControllerCompletedHandler>(
        [tab, initialUri, replaceTab](HRESULT result, ICoreWebView2Controller* controller) -> HRESULT {
            int currentIndex = FindTab(tab.get());
            auto removeFailedTab = [&]() {
                int failedIndex = FindTab(tab.get());
                if (failedIndex < 0) return;
                g_tabs.erase(g_tabs.begin() + failedIndex);
                if (g_tabs.empty()) {
                    g_activeTab = -1;
                } else if (g_activeTab > failedIndex) {
                    --g_activeTab;
                } else if (g_activeTab >= static_cast<int>(g_tabs.size())) {
                    g_activeTab = static_cast<int>(g_tabs.size()) - 1;
                }
                SetActiveTab(g_activeTab);
            };
            if (currentIndex < 0) return S_OK;
            ...
            auto messageCallback = Callback<ICoreWebView2WebMessageReceivedEventHandler>(
                [tab](ICoreWebView2* sender, ICoreWebView2WebMessageReceivedEventArgs* args) -> HRESULT {
                    ...
                    HandleV6PasswordMessage(tab.get(), args);
                    return S_OK;
                });
            ...
            auto navigationCallback = Callback<ICoreWebView2NavigationCompletedEventHandler>(
                [tab](ICoreWebView2* sender, ICoreWebView2NavigationCompletedEventArgs* args) -> HRESULT {
                    ...
                    InjectV6PasswordDetector(tab.get());
                    return S_OK;
                });
            EventRegistrationToken navigationToken{};
            ...
            if (replaceTab && replaceTab.get() != tab.get()) {
                int oldIndex = FindTab(replaceTab.get());
                if (oldIndex >= 0) {
                    if (replaceTab->controller) replaceTab->controller->Close();
                    replaceTab->webview.Reset();
                    replaceTab->controller.Reset();
                    g_tabs.erase(g_tabs.begin() + oldIndex);
                    if (g_activeTab > oldIndex) --g_activeTab;
                    else if (g_activeTab >= static_cast<int>(g_tabs.size())) g_activeTab = static_cast<int>(g_tabs.size()) - 1;
                    SavePersistentData();
                }
            }
...
std::wstring HResultLabel(HRESULT hr) {
    wchar_t buffer[32]{};
    swprintf_s(buffer, _countof(buffer), L"0x%08lX", static_cast<unsigned long>(hr));
    return buffer;
}

bool CheckWebView2Runtime() { LPWSTR version=nullptr; HRESULT hr=GetAvailableCoreWebView2BrowserVersionString(nullptr,&version); if(SUCCEEDED(hr)&&version){CoTaskMemFree(version);return true;} MessageBoxW(g_main,(L"Microsoft WebView2 Runtime is required. Install the Evergreen WebView2 Runtime, then start Pane Browser v7.0.0 again.\n\nHRESULT: " + HResultLabel(hr)).c_str(),L"Pane Browser v7.0.0",MB_OK|MB_ICONERROR); if(version)CoTaskMemFree(version); return false; }
    ...
    auto environmentCallback = Callback<ICoreWebView2CreateCoreWebView2EnvironmentCompletedHandler>(
        [](HRESULT result, ICoreWebView2Environment* environment) -> HRESULT {
            if (FAILED(result) || !environment) {
                MessageBoxW(g_main, (L"Microsoft WebView2 Runtime was not found or could not be initialized.\n\nHRESULT: " + HResultLabel(result)).c_str(), L"Pane Browser v7.0.0", MB_OK | MB_ICONERROR);
                return FAILED(result) ? result : E_FAIL;
            }
...
void RefreshRuntimeTheme() {
    ...
    for (auto& tab : g_tabs) {
        ...
        } else if (tab->passwordPage) {
            tab->webview->NavigateToString(PasswordsHtml().c_str());
...
void ApplyCustomTitleBar() {
    ...
    if (g_settings.customTitleBar && !g_browserFullscreen) {
...
void ShowNativeContextMenu(POINT pt) {
    ...
    AppendMenuW(menu, MF_SEPARATOR, 0, nullptr);
    AppendMenuW(menu, MF_STRING, kContextViewSource, L"View Source");
    AppendMenuW(menu, MF_STRING, kContextInspect, L"Inspect");
    AppendMenuW(menu, MF_SEPARATOR, 0, nullptr);
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    ...
    case WM_PAINT: {
        if (g_settings.customTitleBar && !g_browserFullscreen) {
            ...
            RECT text{14, 0, r.right - 150, 32}; DrawTextW(dc, L"Pane Browser v7.0.0", -1, &text, DT_SINGLELINE | DT_VCENTER | DT_LEFT);
    ...
    case WM_SYSCOLORCHANGE:
        ...
        return 0;
    case kOpenViewSourceMessage:
        OpenViewSource();
        return 0;
    case kOpenDevToolsMessage:
        OpenDevTools();
        return 0;
    ...
    case kUpdateResultMessage: {
        ...
        g_updateCheckRunning.store(false);
        if (!result) return 0;
        if (!result->requestSucceeded) { MessageBoxW(g_main, L"No release information was available.", L"Pane Browser v7.0.0", MB_OK | MB_ICONINFORMATION); return 0; }
        if (result->tag == L"v5.0.0" || result->tag == L"5.0.0") { MessageBoxW(g_main, L"You are using the latest Pane Browser v7.0.0 release (v5.0.0).", L"Pane Browser v7.0.0", MB_OK | MB_ICONINFORMATION); return 0; }
        std::wstring prompt = L"Latest GitHub release: " + result->tag + L"\nOpen the releases page to download it?";
        if (MessageBoxW(g_main, prompt.c_str(), L"Pane Browser v7.0.0", MB_YESNO | MB_ICONINFORMATION) == IDYES) ShellExecuteW(g_main, L"open", L"https://github.com/xizar280513/Pane-Browser/releases", nullptr, nullptr, SW_SHOWNORMAL);
        return 0;
    }
    case WM_TIMER:
    ...
    case WM_COMMAND:
        ...
        case kContextViewSource: OpenViewSource(); return 0;
        case kContextInspect: OpenDevTools(); return 0;
...
int WINAPI wWinMain(HINSTANCE instance, HINSTANCE, PWSTR, int show) {
    ...
    g_main = CreateWindowExW(0, wc.lpszClassName, L"Pane Browser v7.0.0", WS_OVERLAPPEDWINDOW,
    ...
    if (FAILED(hr)) { MessageBoxW(g_main, (L"WebView2 initialization failed.\n\nHRESULT: " + HResultLabel(hr)).c_str(), L"Pane Browser v7.0.0", MB_OK | MB_ICONERROR); DestroyWindow(g_main); CoUninitialize(); return 1; }
